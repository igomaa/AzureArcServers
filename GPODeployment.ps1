...
function Deploy-Agent {
    ...
    if ($isDeployed) {
        $logMessage = "Azure Arc Server enabled agent is deployed , exit process"
        $logMessage >> $logpath
        exit
    }
    else { 
        ...
        if ($exitCode -eq 0) {
            ...
            $message = & "$env:ProgramW6432\AzureConnectedMachineAgent\azcmagent.exe" connect --config "$BitsDirectory\$configFilename"
            $message >> $logpath 
        }
        else {
